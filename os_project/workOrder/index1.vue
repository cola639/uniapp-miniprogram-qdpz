<template>
  <view class="contaier">
    ...
    <!-- 切换 -->
    <view class="typeBox">
      <view @click="typeClick(0)" class="typeBtn" :class="typeNum == 0 ? 'typeActive' : ''"
        >已收货(8)</view
      >
      <view @click="typeClick(1)" class="typeBtn" :class="typeNum == 1 ? 'typeActive' : ''"
        >未收货(12)</view
      >
    </view>
        ...
        <view class="carContentBox">
          <view class="carContentLeftBox">
            <view class="carContent">
              <view class="carTitle text-grey"
                >发货标号：<text class="m0">{{ item.grade }}</text></view
              >
              <view class="carTitle text-grey"
                >偏差率：<text class="m1">{{ item.deviation }}</text></view
              >
            </view>
            <view class="carContent">
              <view class="carTitle text-grey"
                >发货算量：<text class="m0">{{ item.issue }}</text></view
              >
              <view class="carTitle text-grey"
                >实际算量：<text class="m0">{{ item.actual }}</text></view
              >
            </view>
          </view>
          <view class="carContentRightBox">
            <view
              @click="goIndex2()"
              class="cu-tag radius light"
              :class="item.btnType == 1 ? 'bg-green' : 'bg-orange'"
              >自动收货</view
            >
          </view>
        </view>
