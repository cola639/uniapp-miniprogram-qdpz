  ...
  <view style="padding-bottom: 150rpx">
    <view class="mainBox bg-white">
      <view
        ...
        style="position: fixed; left: 40rpx; top: 100rpx; color: #000000"
      >
      ...
      <view class="scBox padding">
        ...
        <view class="margin-top">
          ...
          <view class="text-black text-smdf margin-top-xxs"
            >点击下方按钮购票，使用【趣滑小程序】下单后，凭商家短信凭证码换票入园</view
          >
        </view>
        <view class="margin-top-sm">
          <view class="text-blue text-df text-bold">预定限制</view>
          <view class="text-black text-smdf margin-top-xxs"
            >本产品仅限成人预定，儿童请自行到华山冰雪世界滑雪场购票</view
          >
        </view>
        <view class="margin-top-sm">
          <view class="text-blue text-df text-bold">门票说明</view>
          <view class="text-black text-smdf margin-top-xxs"
            >滑雪票含：雪鞋、雪板、雪仗，节假日不可用</view
          >
        </view>
        <view class="margin-top-sm">
    ...
    <view class="bottomBox" style="z-index: 98">
      <view class="priceNum">
        <image
          class="checkOutLogo"
          src="https://cdn.zhoukaiwen.com/qh_logo_tm2.png"
          mode="widthFix"
        ></image>
    ...
    >
      <view class="cu-dialog" @tap.stop="" style="position: relative; padding-top: 15rpx">
        ...
        <view class="cu-card article no-card">
          <view
            class="cu-item padding-top-sm shadow solid-bottom"
          ...
          >
            <view class="content">
              ...
              <view class="desc padding-tb-xs">
                <view
                  class="text-cut text-black text-bold text-df flex justify-between padding-right-xs"
                >
                  华山冰雪世界
                </view>
                ...
                <view v-if="ticketIndex != -1" class="text-sm text-left text-blue">
                  <text class="text-grey margin-right-sm"
                    >原价：<text style="text-decoration: line-through"
                      >￥{{
                        ticketList[ticketIndex].Yprice ? ticketList[ticketIndex].Yprice : '0'
                      }}</text
                    ></text
                  >
                  优惠价：￥<text class="text-bold text-lgx">{{
                    ticketList[ticketIndex].price ? ticketList[ticketIndex].price : '0'
                  }}</text>
                </view>

                <view v-if="ticketIndex == -1" class="text-left text-sm">
                  请选择：<view class="cu-tag bg-blue light sm round" style="margin-top: -6rpx"
                    >票种</view
                  >
                </view>
                <view v-if="ticketIndex != -1" class="text-left text-sm">
                  已选：<view class="cu-tag bg-blue light sm round" style="margin-top: -6rpx">{{
                    ticketList[ticketIndex].name
                  }}</view>
                </view>
              </view>
            </view>
        ...
        <view class="ticketBox padding-lr bg-white text-left">
          <view
            @click="ticketBtn(index)"
            :class="ticketIndex == index ? 'bg-blue' : ''"
        ...
        <view
          v-if="ticketList[ticketIndex].state"
          class="text-grey padding-lr padding-tb-xs text-left"
          >使用说明：{{ ticketList[ticketIndex].state }}</view
        >
        <view class="cu-bar bg-white">
          <view class="action">
            <text class="cuIcon-titles text-blue"></text>
        ...
        <view class="padding padding-bottom-lg bg-white">
          <view @click="goPay" class="bg-blue padding-sm radius text-center"> 立即购买 </view>
        </view>
      </view>
    </view>
  </view>
