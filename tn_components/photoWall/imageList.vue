<template>
  <view>
    ...
    <scroll-view scroll-x class="bg-white nav" scroll-with-animation :scroll-left="scrollLeft">
      <view
        class="cu-item"
        :class="index == TabCur ? 'text-blue cur text-shadow' : ''"
      ...
      >
        <text
          :class="index == TabCur ? 'cuIcon-timefill' : 'cuIcon-time'"
    ...
    <view class="bg-white padding-sm padding-bottom">
      <view class="grid col-3 grid-square padding-bottom-xl">
        ...
        >
          <image
            v-show="!item.isBuy"
            @tap.stop="addBtnFun(item, index)"
            class="addDelBtn"
            src="https://cdn.zhoukaiwen.com/icon_wxz.svg"
            mode="widthFix"
          ></image>
          <image
            v-show="item.isBuy"
            class="addOkBtn"
            src="https://cdn.zhoukaiwen.com/icon_xz.svg"
            mode="widthFix"
          ></image>
        </view>
      </view>
    ...
    <view class="bottomBox">
      <view class="priceNum">
        <view
          :class="animation"
          style="right: 215rpx !important; top: 18rpx !important"
          class="cu-tag badge"
        >
          {{ checkOutList.length }}
        </view>
        <image
          @click="showModal('bottomModal')"
          class="checkOutLogo"
          src="https://cdn.zhoukaiwen.com/qh_logo_tm2.png"
          mode="widthFix"
        ></image>
        <view class="text-sm text-grey"
          >原价：<text style="text-decoration: line-through">￥299.00</text></view
        >
        <view class="text-smdf text-white"
          >优惠价：<text class="text-df text-bold text-orange">￥99.00</text></view
        >
      </view>
    ...
    <view class="cu-modal bottom-modal" :class="modalName == 'bottomModal' ? 'show' : ''">
      <view class="cu-dialog" style="padding-bottom: 100rpx">
        <view class="cu-bar bg-white">
          <view v-if="checkOutList.length > 0" class="action text-blue" @tap="delImg"
            ><text class="cuIcon-delete"></text>清空购物车</view
          >
          <view v-if="checkOutList.length < 1" class="action text-grey"
            ><text class="cuIcon-delete"></text>清空购物车</view
          >
        </view>
        <view class="padding">
          <view
            v-if="checkOutList.length < 1"
            class="text-center padding-bottom-lg text-grey text-lg"
            >请先添加照片
          </view>
