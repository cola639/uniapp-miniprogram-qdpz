  ...
  <view>
    ...
    <view class="bannerBg" @tap="plateShow = true">
      ...
      <view class="carNumBox">
        <view class="plate-box" :class="PageCur == '1' ? '' : 'green'">
          <uni-label class="uni-label-pointer focus" id="input0" data-index="0">{{
            plateNo.substr(0, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input1" data-index="1">{{
            plateNo.substr(1, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input2" data-index="2">{{
            plateNo.substr(2, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input3" data-index="3">{{
            plateNo.substr(3, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input4" data-index="4">{{
            plateNo.substr(4, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input5" data-index="5">{{
            plateNo.substr(5, 1)
          }}</uni-label>
          <uni-label class="uni-label-pointer" id="input6" data-index="6">{{
            plateNo.substr(6, 1)
          }}</uni-label>
          <uni-label
            class="uni-label-pointer"
            :class="PageCur == '1' ? 'xinnengyuan' : ''"
            id="input7"
            data-index="7"
            >{{ plateNo.substr(7, 1) }}</uni-label
          >
        </view>
      </view>
    </view>
    <view style="margin-top: 110rpx" class="cu-list menu">
      <view class="cu-item">
        <view class="content">
          <image
            src="https://zhoukaiwen.com/img/icon/carP.png"
            class="png"
            mode="aspectFit"
          ></image>
        ...
        <view class="action">
          <view v-if="PageCur == '1'" class="cu-tag round bg-blue light">{{
            plateNo | plateNoF
          }}</view>
          <view v-if="PageCur != '1'" class="cu-tag round bg-green light">{{
            plateNo | plateNoF
          }}</view>
        </view>
      </view>
      ...
      <view class="cu-item">
        ...
        <view class="action">
          <text v-show="plateNo != ''" :class="PageCur == '1' ? 'text-blue' : 'text-green'"
            >临停车辆</text
          >
        </view>
      </view>
      ...
      <view class="cu-item">
        <view class="content">
          <image
            src="https://zhoukaiwen.com/img/icon/clock2.png"
            class="png"
            mode="aspectFit"
          ></image>
      ...
      <view class="cu-item">
        <view class="content">
          <image
            src="https://zhoukaiwen.com/img/icon/clock1.png"
            class="png"
            mode="aspectFit"
          ></image>
    ...
    <view class="cu-list menu">
      <view class="cu-item">
        <view class="content padding-tb-sm">
          ...
          <view class="text-gray"> 当前空车位总数：1200 </view>
          <view class="text-gray"> 当前时段：8:00 - 18:00，收费：6元/小时 </view>
        </view>
      </view>
    </view>
    <uni-view class="padding flex flex-direction">
      <uni-button
        ...
        :class="PageCur == '1' ? 'bg-blue' : 'bg-green'"
        >确认支付 ¥30.00</uni-button
      >
      <uni-button
        v-show="plateNo == ''"
        @tap="plateShow = true"
