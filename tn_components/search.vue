      ...
      <view class="station-list" :style="{ height: `${showList ? 860 : initHeight}rpx` }">
        <scroll-view scroll-y="true" :style="{ height: `${showList ? 830 : initHeight}rpx` }">
          <view class="item" v-for="item in stationList" :key="item.id">
            <view class="item-img">
              <image
                :src="item.photos.length <= 0 ? placeholder : item.photos[0].url"
                mode=""
              ></image>
            </view>
            <view class="item-info">
              ...
              <view class="item-nav">
                <view
                  >距离:<text>{{ (item.distance / 1000) | formatLatlon }}km</text></view
                >
                <view
                  >电话:<text>{{ item.tel || '暂无' }}</text></view
                >
